...
def evaluate(
    ...
    *,
    ...
    valid = sum(
        bool(values)
        and max(values) - min(values) > flat_span
        and not any(abs(value) >= rail_abs for value in values)
    ...
    return {
        ...
        "valid_window_ratio": 1.0 if channel_count and valid / channel_count >= 0.5 else 0.0,
    }
