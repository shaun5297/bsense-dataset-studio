...
REQUIRED_KINDS = ("eeg", "fnirs", "motion", "metric", "heart_rate", "biomultilite_marker", "general_metric", "experiment_marker")
    ...
    def start(self, *, timeout: float = 5.0) -> None:
        ...
        try:
            for stream_id, kind in enumerate(self.required_kinds, 1):
                ...
                states.append(_State(StreamInlet(info, max_buflen=60), stream_id, descriptor.channel_count, int(info.channel_format())))
        except Exception:
            writer.close()
            raise
            ...
            try:
                while not self._stop.is_set():
                    ...
                    for state in states:
                        ...
                        if timestamps:
                            received = True
                            writer.write_samples(state.stream_id, timestamps, samples, state.channel_count, state.channel_format)
