...
def main() -> None:
    ...
    parser.add_argument(
        "--quality-profile", choices=("strict", "pilot_clean_windows"), default="strict"
    )
    ...
    eegnet_path = (
        args.eegnet_output or (root / "derived" / "eegnet" / "eeg_windows.npz")
    ).resolve()
    ...
    split_path.write_text(
        json.dumps(manifest["subject_splits"], ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    print(output_path)
    print(manifest_path)
